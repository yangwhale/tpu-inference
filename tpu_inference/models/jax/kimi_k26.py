...
# Register kimi_k25 config type with transformers (K2.6 has same architecture as K2.5).
...
# Kimi K2.6 1T MoE model configuration.
# Based on moonshotai/Kimi-K2.6 config.json (verified 2026-04-25, see plan v0.4.3).
# K2.6 actually reuses DeepseekV3ForCausalLM model class via text_config.architectures.
...
ffw_intermediate_size: int = 18432  # intermediate_size (dense MLP, layer 0 only) in HF config
...
first_k_dense_replace: int = 1  # K2.6: only layer 0 is dense MLP, layers 1-60 are MoE

num_shared_experts = 1
rope_theta = 50000.0
# K2.6 uses YaRN RoPE (factor=64, original_max_position=4096 → 256K context).
# RoPE adaptation deferred to Round 4 (current code still inherits GLM-5.1's InterleavedRotaryEmbedding,
# which uses standard RoPE without YaRN — must be replaced with YaRN for K2.6's 256K context).
q_lora_rank = 1536
kv_lora_rank = 512
...
class KimiK26BaseAttention(JaxModule):
    """
    Base class containing shared logic for Kimi K2.6 MLA Attention mechanisms.
    ...
    def __post_init__(self, rngs: nnx.Rngs):
        ...
        # Kimi K2.6 uses YaRN RoPE (factor=64). Same mscale formula as DeepSeek V3.
        # When scaling_factor > 1, attention output magnitude needs adjustment to
        # preserve numerical stability with extended context.
        # Reference: deepseek_v3.py:170-176
...
@dataclass(kw_only=True)
class KimiK26Attention(KimiK26BaseAttention):
...
class MLAEinsum(JaxEinsum):
    ...
    def load_weights(self, weights):
        ...
        # K2.6: attention is unquantized (BF16), so self.weight is already BF16,
        ...
        with cpu_mesh_context():
            if is_unquantized:
                ...
                # JaxEinsum kernel_shape was created as (A, N*head_dim), and
                # vLLM weight loader keeps the same orientation since we don't
            ...
            k_ANH, v_ANH = jnp.split(dequantized_weight, [qk_nope_head_dim],
                                     axis=-1)
            if is_unquantized:
                # No quantize back — just keep BF16
                k_ANH_weight = k_ANH
                v_ANH_weight = v_ANH
                k_N1A_scale = None
                v_N1H_scale = None
...
@dataclass(kw_only=True)
class KimiK26MLA(KimiK26BaseAttention):
...
@dataclass(kw_only=True)
class KimiK26FusedMoe(JaxMoE):
    """
    Corresponds to vLLM's KimiK26FusedMoe.
    ...
    """
    shared_experts: Optional[KimiK26MLP] = None
...
class KimiK26Layer(JaxModule):
    """Jax implementation of Kimi K2.6 MoE layer (same structure as DeepSeek V3)."""
    ...
    def __init__(self,
        ...
        # shared experts
        self.shared_experts = KimiK26MLP(
            dtype=dtype,
        ...
        self.experts = KimiK26FusedMoe(
            ...
            # It's abnormal prefix here because we are using dataclass for KimiK26FusedMoe and JaxMoe.
...
class KimiK26DecoderLayer(JaxModule):
    """
    Implements the DecoderLayer for Kimi K2.6 MoE.
    """
    ...
    def __init__(
            ...
            self_attn: Union[KimiK26Attention, KimiK26MLA],

            # MLP can be either the Dense MLP (for first k layers) or KimiK26FusedMoe
            mlp: nnx.Module | KimiK26FusedMoe | KimiK26MLP,
            prefix: str = ""):
...
class KimiK26Model(JaxModule):
    ...
    def __init__(self,
        ...
        logger.info(f"Is using MLA kernel in Kimi K2.6: {self.use_mla_kernel}")
        ...
        # K2.6 is a multimodal wrapper (KimiK25ForConditionalGeneration).
        # All language model config (hidden_size, num_layers, rope_*, etc.) lives
        # under text_config. Use it if present.
        hf_config_raw = vllm_config.model_config.hf_config
        hf_config = getattr(hf_config_raw, "text_config", None) or hf_config_raw
        ...
        # K2.6 RoPE: YaRN with factor=64, original_max_position=4096 → 256K context.
        # Pull rope_scaling from text_config (multimodal), fall back to defaults.
        ...
        self.rope_emb = DeepseekScalingRotaryEmbedding(
            ...
            scaling_factor=rope_scaling.get("factor", 64.0),
            dtype=dtype,
            beta_fast=rope_scaling.get("beta_fast", 32),
            beta_slow=rope_scaling.get("beta_slow", 1),
            mscale_value=rope_scaling.get("mscale", 1.0),
            mscale_all_dim=rope_scaling.get("mscale_all_dim", 1.0),
        )

        def _create_deepseek_attention(
                i: int) -> Union[KimiK26MLA, KimiK26Attention]:
        ...
        # Kimi K2.6: num_hidden_layers=78 (standard layers 0-77), MTP layer 78 is separate.
...
class KimiK26ForCausalLM(JaxModule, LoadableWithIterator):
    ...
    def __init__(self, vllm_config: VllmConfig, rng_key: jax.Array,
        ...
        if self.model.is_last_rank:
            ...
            self.lm_head = JaxEinsum(
                ...
                # Kimi K2.6 doesn't quantize lm_head.
                quant_config=None,
                prefix="lm_head",
            )
        else:
    ...
    @staticmethod
    def _strip_kimi_k26_prefix(weights: Iterable) -> Iterable:
        """K2.6 是 multimodal wrapper（KimiK25ForConditionalGeneration），
        所有语言模型权重以 'language_model.' 开头，需要 strip 掉再传给 loader。
        同时 skip vision_tower / mm_projector（vision encoder Phase 3，暂不支持）。

        Round 2 (2026-04-25)：只 strip + skip，不处理 INT4 unpack（Round 3）。
        """
    ...
    def load_weights(self, weights: Iterable) -> set[str]:
        ...
        # K2.6: max 61 transformer layers, no MTP (num_nextn_predict_layers=0).
        # Upper bound 62 is safe (includes the rare case of layers.61 metadata).
        end_ignore_layer_num = 62
        skip_substrs = [
            f"layers.{i}"
            for i in range(start_ignore_layer_num, end_ignore_layer_num)
        ]
        # Skip DSA indexer weights (K2.6 doesn't use DSA, but inherited from GLM-5.1 plumbing)
        skip_substrs += ["indexer"]
        skip_prefixes = ["lm_head"] if not hasattr(self, 'lm_head') else []
        # Inherited from GLM-5.1: MTP-specific top-level weights
        # (K2.6 has no MTP, but skip them defensively in case checkpoint includes them)
        skip_prefixes += ["eh_proj", "enorm", "hnorm", "shared_head"]

        # K2.6: strip 'language_model.' prefix + skip vision_tower / mm_projector
        weights = self._strip_kimi_k26_prefix(weights)
